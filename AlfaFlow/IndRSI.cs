...
namespace AlfaFlow
{
	public class IndRSI: ITemporalObserver, IParameterized, IDisposable
	{
		string _token;
		string _out_token;
		TemporalContextManager _manager;
		IDisposable _unsubscribe;
		public IndRSI(TemporalContextManager manager, string token)
		{
			_manager = manager;
			_token = token;
			_period = 1;
		...
		public string[] Tokens()
		{
			return new[] 
			{
				String.Concat(_token, "#RSI#", String.Format("Period={0}", _period)),
			};
		}
