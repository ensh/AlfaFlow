...
namespace AlfaFlow
{
	public class IndATR: ITemporalObserver, IParameterized, IDisposable
	{
		string _token;
		string _out_token;
		TemporalContextManager _manager;
		IDisposable _unsubscribe;
		public IndATR(TemporalContextManager manager, string token)
		{
			_manager = manager;
			_token = token;
			_period = 1;
		...
		public string[] Tokens()
		{
			return new[] 
			{
				String.Concat(_token, "#ATR#", String.Format("Period={0}", _period)),
			};
		}
